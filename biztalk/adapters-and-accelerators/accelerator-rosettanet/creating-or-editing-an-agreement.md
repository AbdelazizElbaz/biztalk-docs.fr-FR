...
|**Protocole**|**Encodage**|Le protocole utilisé pour coder toutes les parties (si le **encoder toutes les parties** zone est `True`) ou la pièce jointe (si le **encoder toutes les parties** zone est `False`).<br /><br /> Peut être **8 bits**, **base 64** (la valeur par défaut), ou **quoted printable**.|  
...
|**Propriétés personnalisées**|**Value**|Valeur de la propriété personnalisée.|  
...
4.  Dans le **\< ***nom du contrat*** \>** boîte de dialogue des propriétés du **général** et **propriétés Contact** onglets, modifiez les paramètres en fonction des besoins. Pour plus d'informations sur ces paramètres, consultez le tableau précédent.  
