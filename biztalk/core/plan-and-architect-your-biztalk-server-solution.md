---
title: Planifier et concevoir votre solution BizTalk Server | Documents Microsoft
ms.custom: ''
ms.date: 06/08/2017
ms.prod: biztalk-server
ms.reviewer: ''
ms.suite: ''
ms.tgt_pltfrm: ''
ms.topic: article
f1_keywords:
- bts10.concepts.planningandimplementation
helpviewer_keywords:
- planning
- architecture
ms.assetid: 81a72da7-c2ee-4be3-9756-7c7dc8e20ea9
caps.latest.revision: 14
author: MandiOhlinger
ms.author: mandia
manager: anneta
ms.openlocfilehash: f6b6b3afc568f4f761f4941af00cebdb7af90578
ms.sourcegitcommit: cb908c540d8f1a692d01dc8f313e16cb4b4e696d
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 09/20/2017
---
# <a name="plan-and-architect-your-biztalk-server-solution"></a>Planifier et concevoir votre solution BizTalk Server
Cette section fournit des informations sur BizTalk Server qui vous permettent de planifier votre environnement.  
  
## <a name="in-this-section"></a>Dans cette section  
  
-   [Architecture de BizTalk Server](../core/biztalk-server-architecture.md)  
  
-   [Performances et planification de capacité](../core/performance-and-capacity-planning.md)  
  
-   [Planification de la haute disponibilité](../core/planning-for-high-availability3.md)  
  
-   [Planification de la sécurité](../core/planning-for-security.md)  
  
-   [Scénarios pour les Solutions d’entreprise](../core/scenarios-for-business-solutions.md)  
  
-   [Comment BizTalk Server traite les Messages volumineux](../core/how-biztalk-server-processes-large-messages.md)  
  
-   [Conception des architectures système pour BizTalk Server](../core/designing-the-system-architectures-for-biztalk-server.md)