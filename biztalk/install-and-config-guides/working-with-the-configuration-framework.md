...
## <a name="configuration-framework-command-line-parameters"></a>Paramètres de ligne de commande de Configuration Framework  
 Le tableau suivant décrit les paramètres que vous pouvez exécuter dans une ligne de commande au sein de Configuration Framework.  
  
|Paramètre de ligne de commande|Description|  
|----------------------------|-----------------|  
|**/U**|Annule la configuration de toutes les fonctionnalités. **Remarque :** Vous pouvez supprimer les bases de données BizTalk Server après avoir annulé la configuration des fonctionnalités de BizTalk Server. Ne supprimez pas les bases de données BizTalk Server avant d'exécuter cette commande. **Remarque :** Si vous voulez supprimer les bases de données BizTalk Server après avoir annulé la configuration des fonctionnalités de BizTalk Server, il est recommandé d’utiliser la commande **net stop winmgmt** pour arrêter le service Windows Management Instrumentation (WMI), éventuellement en cours d’exécution.|  
|**/S**|Configuration silencieuse.<br /><br /> Vous devez également transmettre le chemin d'accès complet au fichier XML de configuration qui contient les fonctionnalités à installer et configurer. Si **/s** n’est pas transmis, l’outil est exécuté en mode interface utilisateur (IU).|  
|**/L**|-   Définit le chemin d’accès complet au fichier journal (facultatif).|  
|**/H**|-   Affiche les paramètres de ligne de commande valides.|  
  
## <a name="sample-configuration-xml-file"></a>Exemple de fichier XML de configuration  
 Un exemple de fichier XML de configuration est fourni ci-dessous avec toutes les fonctionnalités installées. Pour créer votre propre fichier de configuration, utilisez l’option Configuration personnalisée sur un serveur [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)], confirmez que tous les composants se sont exécutés correctement, puis choisissez **Exporter la configuration** dans la fenêtre Configuration. Vous pouvez ensuite utiliser le fichier de configuration exporté pour configurer d’autres serveurs [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)].  
